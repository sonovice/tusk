...
///
/// `BytesText::new()` escapes `"` → `&quot;` which is valid but causes problems
/// when the reader uses `trim_text(true)`: whitespace after `&quot;` is trimmed
/// on re-parse.
...
impl<W: Write> MusicXmlWriter<W> {
    ...
    /// entity-encoded: `trim_text(true)` eats whitespace after `&quot;` on re-parse.
