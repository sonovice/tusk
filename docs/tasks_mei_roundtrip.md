...
- [ ] [SERIALIZER_BUG] StaffDef: key.mode attribute not serialized - AttStaffDefAnl attributes (key.accid, key.mode, key.pname) missing from collect_all_attributes (source: Bach-JC_Fughette_No2.mei)
- [ ] [DESERIALIZER_BUG] PgHead: table element not parsed - skipped instead of deserialized (source: Bach-JC_Fughette_No2.mei)
